...
def main():
    ...
    for year in years:
        ...
        for m in range(1, 13):
            ...
            monthly_all.append({
                ...
                "month": m,
                "monthly_return": mr,
                "active_month": "true" if active else "false"
            })
            nav_prev = nav_eom

        # --- Yearly summary availability/YTD logic ---
        months_with_any_data = set(eom.keys()) | set(amf.keys())
        last_month_with_data = max(months_with_any_data) if months_with_any_data else 0
        current_year = pd.Timestamp.utcnow().year
        if last_month_with_data == 0:
            months_in_year_available = 0
            is_ytd = True
        elif year == current_year:
            months_in_year_available = int(last_month_with_data)
        ...
        if ARGS.debug_mdd_year and ARGS.debug_mdd_year == year:
            ...
            if not dfdbg.empty:
                ...
                dfdbg.to_csv(OUTPUT_DIR / f"mdd_debug_{year}.csv", index=False)
        ...
        if tr_df is None or tr_df.empty:
            ...
            if tr.empty:
                ...
                if len(nonzero) == 0:
                    ...
                    pf = None if neg_sum == 0 else float(pos_sum / abs(neg_sum))
    ...
    for c in ["annual_return_calendar", "intramonth_max_drawdown_intra_year", "eom_max_drawdown_intra_year", "active_months_share", "win_rate"]:
        ys[c] = ys[c].apply(fmt4)
    ys["profit_factor"] = ys["profit_factor"].apply(fmt2)

    for c in ["insufficient_months", "insufficient_active_months", "insufficient_negative_months", "insufficient_trades", "is_ytd"]:
        ys[c] = ys[c].apply(lambda x: "true" if bool(x) else "false")

    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    ys.to_csv(OUTPUT_DIR / "yearly_summary.csv", index=False)

    # ---- Write monthly_returns.csv once (overwrite, no duplicates) ----
    monthly_df = pd.DataFrame(monthly_all)
    if not monthly_df.empty:
        monthly_df = monthly_df[["year","month","monthly_return","active_month"]].sort_values(["year","month"]).reset_index(drop=True)
        monthly_df["monthly_return"] = monthly_df["monthly_return"].apply(fmt4_strict)
        monthly_df.to_csv(OUTPUT_DIR / "monthly_returns.csv", index=False)
