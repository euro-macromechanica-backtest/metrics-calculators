...
----------------------------------
Single-file calculator for compounding base metrics (compounding_eoy_soy_base_100k).

STRICT POLICIES:
- No deduplication or hidden preprocessing. Input order preserved except where methodology requires sorting (RAW equity for DD).
- Inputs from ./input: semicolon-delimited CSV/TXT.
- Trades attribution = by close timestamp month/year.
- pnl_pct in trades is interpreted as PERCENT units (e.g., 0.47132 => 0.47132%).
- Risk per trade is provided via --risk-pct (e.g., 1.0 for 1%).

Run examples:
    python compounding_base_metrics_calculator.py --risk-pct 1.0
    python compounding_base_metrics_calculator.py --risk-pct 1.5 --input-dir ./input --output-dir ./output
